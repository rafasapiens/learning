...
class Login:
    ...
    def conexaoBanco(self):
        ...
        try:
            con = sqlite3.connect (db=BANCO, user=USER, passwd=PASSWD, host=HOST)
            print('Oba! Conseguimos conectar ao banco de dados')
        except sqlite3 as erro:
            print('Oh não! Não conseguimos conectar ao banco de dados.: ',erro)
        return con
